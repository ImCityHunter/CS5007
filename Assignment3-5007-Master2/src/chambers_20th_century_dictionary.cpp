 ...
 */
Dictionary* readChambers_20th_CenturyDictionary(
    ...
    if (found) {
		do {
			...
			if (wordLen != string::npos) {
				...
				dict->insert(make_pair(word, def));
...
void testChambers_20th_CenturyDictionary(void) {
	...
	Dictionary* dict = readChambers_20th_CenturyDictionary(file, firstWord, lastWord);
	pclose(file);
		...
		if (cmd.compare("quit") == 0) {
			...
			if (cmd.back() == '*') { // process wildcard command
				...
				if (itr == dict->end()) { // report no definition
					...
					cout << ((cmd[0] == '?') ? itr->second : itr->first) << endl;
				}
			}
